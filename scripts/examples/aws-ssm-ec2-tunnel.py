#!/usr/bin/env python3
"""AWS SSM → EC2 pre-connect script for fluid-postgres-mcp.

Opens an SSM port-forwarding session to PostgreSQL running on the
target EC2 instance, emits the stdout handshake fluid-postgres-mcp
consumes, then supervises the SSM child. Exit on child death is the
signal to respawn — that is the recovery loop.

Required env: EC2_INSTANCE_ID, EC2_REGION, DB_NAME, DB_USERNAME,
DB_PASSWD.
Optional env: DB_HOST (default localhost), DB_PORT (default 5432),
ASSUME_ROLE_ARN, AWS_PROFILE.
...
RDS-via-EC2-forwarder variant): README §AWS SSM examples.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="aws-ssm-ec2-tunnel.py",
        description=(
            "Generic AWS SSM port-forward pre-connect script for "
            "fluid-postgres-mcp. All deployment-specific values come "
            "from environment variables; see the header docstring."
        ),
    )
    parser.add_argument(
        "--profile",
...
def start_ssm_tunnel(
    ...
    # When remote_host == localhost (PG on the EC2 itself), use the
    # standard port-forward document. For RDS-via-bastion you would
    # normally use AWS-StartPortForwardingSessionToRemoteHost — kept
    # simple here, the example targets PG on the EC2 directly.
...
def main() -> None:
    ...
    _ssm_child, _ssm_session_id = start_ssm_tunnel(
        ...
        remote_host=os.environ.get("DB_HOST", "localhost"),
        remote_port=db_port,
    )
